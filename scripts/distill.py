...
def _student_render(
    student: dict[str, jax.Array], vm: jax.Array, H: int, W: int, intr: tuple[float, float, float, float]
) -> jax.Array:
    fx, fy, cx, cy = intr
...
def main() -> dict:
    ...
    ap.add_argument("--synth-downscale", type=int, default=4,
    ...
    scene = load_scene(args.data, args.downscale, args.eval_every,
                       seed=args.seed, sparse_model=args.sparse_model)
    ...
    print(f"eval {W}x{H} (ds{args.downscale}) | synthetic {Ws}x{Hs} (ds{args.synth_downscale}), "
    ...
    teacher_pf = [psnr(_student_render(teacher, eval_vms[i], H, W, intr), eval_imgs[i])
                  for i in eval_idxs]
    teacher_psnr = float(np.mean(teacher_pf))
    print(f"teacher held-out PSNR (ceiling): {teacher_psnr:.2f} dB {[round(x,2) for x in teacher_pf]}")

    def eval_hook(student: dict[str, jax.Array]) -> float:
        return float(np.mean([psnr(_student_render(student, eval_vms[i], H, W, intr),
                                   eval_imgs[i]) for i in eval_idxs]))

    # --- distill -----------------------------------------------------------------
    info: dict = {}
    t0 = time.perf_counter()
    student = splax.distillation.distill(
        teacher, args.n_student, img_shape=(Hs, Ws), f=fs, c=cs,
        n_views=args.n_views, steps=args.steps, depth_lambda=args.depth_lambda,
        init=args.init, seed=args.seed, viewmats=train_vms, batch=args.batch_size,
        noise_lr=args.noise_lr, log_every=args.log_every, eval_hook=eval_hook, info=info,
    )
    total_wall = time.perf_counter() - t0

    per_frame = [psnr(_student_render(student, eval_vms[i], H, W, intr), eval_imgs[i])
                 for i in eval_idxs]
    final = float(np.mean(per_frame))
    for c in info["curve"]:
        print(f"  step {c['step']:5d}  " +
    ...
    if args.out_ply:
        ...
        splax.write_ply(args.out_ply, student["means"], student["scales"],
                        student["quats"], student["colors"], student["opacities"])
        print(f"wrote {args.out_ply}")

    result = {
        "teacher_ply": args.teacher_ply, "teacher_n": int(tm.shape[0]),
        "teacher_psnr": teacher_psnr, "teacher_per_frame": teacher_pf,
        "n_student": args.n_student, "n_views": args.n_views, "steps": args.steps,
        "batch": args.batch_size, "depth_lambda": args.depth_lambda, "init": args.init,
        "downscale": args.downscale, "synth_downscale": args.synth_downscale,
        "final": final, "per_frame": per_frame,
        "gap": teacher_psnr - final, "wall": total_wall, "fit_wall": info["wall"],
        "curve": info["curve"], "names": [scene["eval_names"][i] for i in eval_idxs],
    }
    if args.out_json:
